...
async function readConfig(remixRoot, serverMode = serverModes.ServerMode.Production) {
  ...
  if (appConfig.future) {
    ...
    let deprecatedFlags = ["unstable_cssModules", "unstable_cssSideEffectImports", "unstable_dev", "unstable_postcss", "unstable_tailwind", "unstable_vanillaExtract", "v2_dev", "v2_errorBoundary", "v2_headers", "v2_meta", "v2_normalizeFormMethod", "v2_routeConvention"];
    let obsoleteFlags = deprecatedFlags.filter(f => f in userFlags);
    if (obsoleteFlags.length > 0) {
      logger.logger.warn(`⚠️ REMIX FUTURE CHANGE: the following Remix future flags are now obsolete ` + `and can be removed from your remix.config.js file:\n` + obsoleteFlags.map(f => `- ${f}\n`).join(""));
    }
  }
  return {
    appDirectory,
